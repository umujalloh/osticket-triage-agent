...
A page carries the same fields and follows the same rules, because PagerDuty sits outside the boundary for the same reasons Slack does. The Events API accepts an arbitrary `custom_details` object, which is exactly where enrichment output would end up if the rule were a per-field judgement rather than a rule, so the agent sends none. The link's text is the URL itself rather than a friendly label, since anyone holding the routing key can raise an incident that looks entirely real.
