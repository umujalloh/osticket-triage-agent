...
docker/             Dockerfile and compose file for the osTicket environment
...
Three things are needed, and the first is easy to miss.

Enable HEC globally: Settings → Data Inputs → HTTP Event Collector → Global
Settings, set All Tokens to Enabled. Creating a token is not enough on its own,
the feature has its own switch.

Create an index named `osticket_triage`.

Create an HEC token with that index in its allowed list. The agent sends events
with sourcetype `osticket:triage:audit`.
...
TRIAGE_HMAC_SECRET=your-generated-secret
...
Edit `docker/.env` and set your own database credentials. These exact values are
used again during the osTicket installer, so keep them to hand.

The Dockerfile removes osTicket's `setup/` directory, which is a live installer
page and a liability once osTicket is installed. That directory has to exist for
the first install, so the first build is done with that line commented out.

Comment out this line in [docker/Dockerfile](docker/Dockerfile):
...
Once the installer finishes, uncomment the line and rebuild:
...
Log into the staff panel at `http://localhost:8080/scp`, then Manage → Plugins.
Open AI Triage Webhook and set it to Active. Add an instance, give it a name,
set its status to Active, and on the Config tab set:

- FastAPI Webhook URL: `http://host.docker.internal:8000/webhook/ticket`
- HMAC Shared Secret: the same value as `TRIAGE_HMAC_SECRET` in `agent/.env`

The container reaches the agent through `host.docker.internal`, which resolves
via the `extra_hosts` entry in
[docker-compose.yml](docker/docker-compose.yml). The agent must be listening on
...
The classifier is evaluated against 30 hand-written tickets with expected labels
in [tests/eval_tickets.json](tests/eval_tickets.json):
